...
module.exports = class extends Vacuum.with(
		...
		if(key === 'state') {
			...
			switch(value) {
				...
				case 'zone-cleaning':
					...
					super.propertyUpdated('zoneName', this.zoneName);
	...
	activateCleaning() {
		return this.call('app_start', [], {
			...
			.then(checkResult).then(setCurrentZone('', []));
	}
	...
	/**
 	 ...
 	 */
 	activateZoneClean(name, zones) {
 		return this.call('app_zoned_clean', zones, {
			refresh: [ 'state' ],
			refreshDelay: 1000
 		})
 			.then(checkResult).then(setCurrentZone(name, zones));
	}
	...
	/**
	 * Sends robot to specified target.
 	 * Takes in an array of target.
 	 * Target example: [26234,26042]
 	 * Each target contains an 2 values. x, y.
 	 * Maps are always 51200 x 51200. The charger/starting location is always the center; so 25600, 25600
 	 */
 	activateGoToTarget(target) {
